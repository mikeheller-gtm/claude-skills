---
name: founder-market-architect
description: "Find the structural, non-product move that could let your company own its market, then pressure-test it before you bet on it. Built on the market-architecture lens of Mike Heller (Floodgate). Use this skill whenever the user wants bold strategy ideas for their own company, or mentions 'market architect', 'run market architect on us', 'dominant strategies', 'how do we win this market', 'what's the bold move here', 'we're stuck competing on features', or asks how to restructure or create a market rather than enter one. Also use it when a founder pastes their deck, website copy, memo, or a pitch recording and asks what the biggest strategic move available to them is. It runs on whatever context exists and asks for the rest."
---

# Market Architect: The Structural Move That Wins Your Market
...
1. What do you sell, and who actually buys it? Name the role and the kind of organization.
2. How do you sell today? Founder-led outbound, inbound, partnerships, self-serve, something else. Rough deal size and sales cycle if you have them.
3. Where are you right now? Number of customers, revenue range, team size. This is not a judgment, it decides which strategies are live now and which need to be sequenced.
4. What is the most ambitious version of this company that you actually believe in? Not the safe version, the one you would say out loud to someone you trust.
5. What gets in your way that is not building the product? The thing that is slow, political, expensive, or that everyone in the industry just accepts.
...
Before generating, establish in a few sentences: what the company sells, who buys it, the current motion, and your read on the true binding constraint (technical or structural). Lead with what is genuinely strong about the business. This diagnosis anchors everything after it, and if it is wrong the founder will tell you, which is useful.
...
- **What has to be true** for it to work, including the result of its kill test
- **The first concrete step** to take in the next 30 days, specific enough to actually start on Monday
- **Closest analog**, when one exists, with a one-line explanation of the analog

Order by potential magnitude, boldest defensible idea first.

### Step 4: Pressure-test every candidate (try to kill it first)

The founder knows their market's ground truth far better than this skill does. One idea they can kill with a single sentence costs credibility for all the others. So run every candidate through these tests. The output of this step is not a caveat paragraph, it is a decision: present as-is, present scoped to where it actually works, sequence it behind an explicit trigger, or cut it.
...
1. **Find the buyer's top problem and make the entire motion about it.** Senior buyers only act on their top two or three priorities. Is the motion anchored on one of them, or is it pitching features into the void?
...
Also check: if the founder describes a product-led motion, does the first value proposition pass the five-second test (fast time to value AND fast time to understand the value)? If not, say plainly that PLG will not work as described, and what the wedge would need to be instead.

If the material warrants a full review, note that `founder-gtm-audit` will go deeper on actual call transcripts.

### Step 6: Where the current plan sits
...
### Step 7: Close with the questions
...
1. **What I see in the business** (diagnosis, strengths first, then the binding constraint)
2. **The lens** (two or three short paragraphs on market architects versus market entrants, so the strategies make sense in context)
3. **The strategies** (3 to 5, ordered by magnitude, each in the format from Step 3)
4. **The go-to-market read** (tight)
5. **Where this plan sits today** (one paragraph)
